package com.example.removebackgrdemo.sticker;
...
import com.example.removebackgrdemo.R;
...
public class StickerView extends FrameLayout {
    ...
    public float rtt,mX,mY,currentX,currentY;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (action) {
            case MotionEvent.ACTION_DOWN:
                ...
                } else if (checkIconTouched(mZoomIcon) && mHandlingSticker != null) { /// ckeck cham vao icon zoom
                    mCurrentMode = ActionMode.ZOOM_WITH_ICON;
                    mMidPoint = calculateMidPoint(); ///tra ve point sticker
                ...
                invalidate();
                break;
                ///Zoom with icon( old goc)(old khoang cach) iocn & mid stick  ||| Zoom with Two finger ( old goc)(old khoang cach) 2 diem mid point=A+B/2
            ...
            case MotionEvent.ACTION_UP:
                ...
                ///////RETURN information
            if(mHandlingSticker!=null) {
                rtt = mHandlingSticker.getCurrentAngle();
                float[] location = new float[8];
                location = getStickerPoints(mHandlingSticker);
                mX = location[0];
                mY = location[1];
                currentX = mHandlingSticker.getCurrentWidth();
                currentY = mHandlingSticker.getCurrentHeight();
                rtt = mHandlingSticker.getCurrentAngle();
                Log.d("LocationRS", location[0] + " " + location[1] + "|||||" + rtt + " " + mHandlingSticker.getWidth() + " " + mHandlingSticker.getHeight() +
                        " X " + mHandlingSticker.getCurrentHeight() + " " + mHandlingSticker.getCurrentWidth());
    ...
    private void handleCurrentMode(MotionEvent event) {
        switch (mCurrentMode) {
            case NONE:
                break;
            case DRAG:
                if (mHandlingSticker != null) { // an vao sticker
